...
router.get('/', async (req, res) => {
  try {
    const query = `
      SELECT p.id, p.title, p.content, p.tags, p.published_date, p.created_at, p.updated_at, p.author, COUNT(c.id) as comment_count 
      FROM blog_posts p 
      LEFT JOIN comments c ON p.id = c.post_id AND c.is_approved = true
      GROUP BY p.id, p.title, p.content, p.tags, p.published_date, p.created_at, p.updated_at, p.author
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const query = 'SELECT id, title, content, tags, published_date, created_at, updated_at, author FROM blog_posts WHERE id = $1';
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const query = `
      ...
      RETURNING id, title, content, tags, published_date, created_at, updated_at, author
    `;
